...
export function AdminCmsSiteContact({ permissions }: { permissions: string[] }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (isAdminOk<{ contact: SiteContactConfig }>(res)) {
      ...
      toast.success("Saved", "Site contact, phones, and WhatsApp templates are updated.");
    } else {
  ...
  return (
    <div className="space-y-8">
      <CmsPageHeader
        ...
        description="Phones, emails, WhatsApp templates, social profiles, and footer legal copy. Manage physical locations separately."
        workflow="live"
      />
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {contact.facilities.length} locations configured — {contact.facilities.filter((f) => f.isPrimary).length} primary,{" "}
                {contact.facilities.filter((f) => !f.isPrimary).length} secondary.
              </p>
            </div>
            <Button variant="outline" size="sm" asChild>
              <Link to="/agaate-admin/locations">Manage locations</Link>
            </Button>
          </div>
        </section>
