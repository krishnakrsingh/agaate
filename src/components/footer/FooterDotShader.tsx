import React, { useMemo, useRef, useState, useCallback, useEffect, type MouseEvent } from "react";
...
const ShaderMaterial = ({
  ...
  const getPreparedUniforms = useCallback(() => {
  ...
  const material = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      uniforms: getPreparedUniforms(),
      glslVersion: THREE.GLSL3,
      blending: THREE.CustomBlending,
      blendSrc: THREE.SrcAlphaFactor,
      blendDst: THREE.OneMinusSrcAlphaFactor,
    });
  }, [source, getPreparedUniforms]);
...
export function FooterDotShader({
  animationSpeed = 0.4,
  ...
  useEffect(() => {
    ...
    const start = hoverIntensityRef.current;
  ...
  return (
    ...
    >
      ...
      {showGradient && (
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[#FAFBFB] to-transparent" />
      )}
    </div>
  );
}
