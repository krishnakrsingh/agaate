  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mt-16 flex flex-wrap gap-x-7 gap-y-4 border-t border-[#174735]/15 pt-7">
          {zones.map((zone, index) => (
            <span
              key={zone}
              className="font-serif text-[1.45rem] tracking-[-0.03em] text-[#143D31]"
            >
              <span className="mr-2 font-jet text-[9px] text-[#5D8D53]">0{index + 1}</span>
              {zone}
            </span>
          ))}
        </div>
