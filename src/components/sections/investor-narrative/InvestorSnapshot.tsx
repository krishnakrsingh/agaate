      ...
      <div className="mx-auto max-w-[1400px]">
        <div className="mb-12 grid gap-8 border-t border-ink/10 pt-6 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-7">
            ...
            <h2 className="mt-5 max-w-4xl font-serif text-[clamp(2.2rem,5vw,4.4rem)] leading-[1.04] text-forest-deep">{t("investorSnapshot.title")}</h2>
          </div>
          <div className="lg:col-span-5">
            <p className="text-[16px] leading-[1.75] text-[#59635D] md:text-[18px]">{t("investorSnapshot.desc")}</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <InlineCta href="/contact">{t("investorSnapshot.cta1")}</InlineCta>
              <InlineCta href="#operating-model" variant="light">{t("investorSnapshot.cta2")}</InlineCta>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-px overflow-hidden rounded-lg border border-[#DDE7E1] bg-[#DDE7E1] sm:grid-cols-2 lg:grid-cols-3">
          {metricIcons.map((Icon, idx) => (
            <div key={idx} className="bg-[#F9FBF8] p-6 md:p-8">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{t(`investorSnapshot.metrics.${idx}.note` as any)}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 grid gap-4 rounded-lg border border-forest/15 bg-forest/[0.03] p-5 md:grid-cols-[1fr_auto] md:items-center">
          <p className="text-sm leading-relaxed text-[#59635D]">{t("investorSnapshot.disclaimer")}</p>
          <Link
