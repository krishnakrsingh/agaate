...
export function UnifiedFarmTechSection() {
  const [activeTab, setActiveTab] = useState<"telemetry" | "ai" | "drone" | "modules">("telemetry");
    ...
    >
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10 space-y-16">
        ...
        <div className="grid gap-12 lg:grid-cols-12 lg:items-center">
            ...
            <TiltCard maxTilt={6} glare={false} className="w-full">
              ...
              >
                ...
                <div className="absolute top-4 left-4 rounded-full bg-[#143d31] px-3.5 py-1 font-mono text-[11px] font-bold text-white shadow-sm flex items-center gap-1.5">
                  ...
                  <span>Solar IoT Probes · Drone Telemetry Mesh</span>
                </div>
              </motion.div>
            </TiltCard>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-center max-w-xl lg:pl-4 space-y-4">
            ...
            <p className="font-sans text-sm sm:text-base leading-relaxed text-[#4f624f]">
              Solar telemetric soil probes, drone multispectral crop scouting, and instant
              smartphone photo diagnosis powered by AI — delivering exact fertigation prescriptions
              in under 15 minutes.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {[
                "Instant photo leaf pest & spore diagnosis",
                "Continuous soil moisture, EC & temp tracking",
                "Automated venturi fertigation loop triggers",
                "Direct WhatsApp agronomist escalation",
              ].map((feat) => (
                <div
                  key={feat}
        ...
        <div className="rounded-[2.5rem] border border-[#143d31]/10 bg-[#f4f8f5] p-6 sm:p-10 shadow-sm space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#143d31]/10 pb-5">
            <div>
              <span className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-[#5d7d37]">
                LIVE IOT & DIAGNOSTIC SUITE
              </span>
              <h3 className="font-display text-2xl font-bold text-[#143d31]">
                Real-Time Telemetry & AI Diagnostic Workbench
              </h3>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[
                { id: "telemetry", label: "Live Telemetry Charts", icon: ChartLineUp },
                { id: "ai", label: "AI Disease Scanner", icon: Camera },
                { id: "drone", label: "Drone Radar & Mesh", icon: NavigationArrow },
                { id: "modules", label: "Hardware Modules", icon: Cpu },
              ].map((tab) => {
                const Icon = tab.icon;
          ...
          {/* Tab Content Display */}
          <div className="pt-2">
            {activeTab === "telemetry" && <TelemetryLiveChart />}
            {activeTab === "ai" && <AiDiagnosticSimulator />}
            {activeTab === "drone" && <FieldNetwork />}
            {activeTab === "modules" && <TechModulesGrid />}
          </div>
        </div>
      </div>
    </section>
