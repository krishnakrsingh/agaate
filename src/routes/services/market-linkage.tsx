        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-7 space-y-8 text-left">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-forest/65 leading-relaxed mt-1">
                    We establish floor rates in contracts, guarding against sudden mandi auction crashes.
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-forest/65 leading-relaxed mt-1">
                    Agaate coordinates direct collection runs from regional hubs to minimize transit waste.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#eef3f0]/50 rounded-[2rem] border border-forest/10 p-8 space-y-4">
              <span className="font-jet text-[9px] tracking-widest text-forest font-bold uppercase block">Partner buyer networks</span>
              <h3 className="font-serif text-2xl text-forest-deep font-bold">Institutional Purchasing Chains</h3>
              <div className="grid grid-cols-3 gap-4 pt-2">
                {[
                  ...
                  <div key={idx} className="bg-white p-4 rounded-xl border border-[#E7ECE8] text-center text-xs font-mono">
          ...
          <div className="lg:col-span-5 bg-bone rounded-[2.5rem] border border-[#E7ECE8] p-8 text-left space-y-8 shadow-sm">
            ...
            <div className="space-y-6">
              <div className="overflow-hidden border border-[#E7ECE8] rounded-2xl bg-white shadow-sm text-xs">
                <div className="grid grid-cols-4 gap-2 bg-[#F9FAF9] p-3 border-b border-[#E7ECE8] font-mono text-[9px] tracking-wider text-forest/40 uppercase font-semibold">
                  <span>COMMODITY</span>
                  <span>MANDI</span>
                  <span>AGAATE</span>
                  <span>BUYER</span>
                ...
                {pricing.map((p, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-4 gap-2 p-3 border-b border-[#E7ECE8] last:border-b-0 items-center"
                  >
                    ...
                    <span className="text-[10px] text-forest/50 truncate font-semibold">{p.buyer.split(" ")[0]}</span>
                  </div>
                ))}
              </div>
              ...
              {inquired ? (
                <div className="p-8 text-center bg-white border border-forest/10 rounded-3xl flex flex-col items-center justify-center min-h-[300px] animate-in fade-in zoom-in-95">
                  ...
                  <p className="text-xs text-forest/70 max-w-xs leading-relaxed">
                    We have logged your buyback inquiry. A market linkages advisor will review your acreage capacity and call with a draft agreement.
                  </p>
                </div>
              ) : (
        ...
        <div className="border-t border-[#E7ECE8] pt-24 text-left">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            <div className="lg:col-span-5 space-y-6">
              <span className="font-jet text-[10px] tracking-widest uppercase text-forest/40 font-bold block">BUYBACK PREVIEW GENERATOR</span>
              ...
              <p className="text-forest/75 text-sm leading-relaxed">
                Input your target crop and acreage scale to compile an instant draft minimum guarantee schedule detailing floor payout models and collection logistics intervals.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-mono tracking-wider text-forest/60 mb-2 uppercase font-semibold">Crop Type</label>
                  <select
                    value={contractCrop}
                    onChange={(e) => {
                ...
                <div>
                  <label className="block text-xs font-mono tracking-wider text-forest/60 mb-2 uppercase font-semibold">Acreage</label>
                  <input
                    type="number"
                    min={1}
                    max={50}
            ...
            <div className="lg:col-span-7">
              {showContractDraft ? (
                <div className="bg-white border border-[#E7ECE8] rounded-[2.5rem] p-8 space-y-6 text-xs font-mono text-left shadow-sm animate-in slide-in-from-right duration-300">
                  <div className="flex justify-between items-center border-b border-[#E7ECE8]/50 pb-3">
                    <span className="font-bold text-forest flex items-center gap-1.5"><FileText className="w-4 h-4" /> DRAFT AGREEMENT SCHEDULING</span>
                    <span className="text-forest/40">CODE: AG-ML-DRAFT</span>
                  </div>

                  <div className="space-y-4 text-forest-deep/80 leading-relaxed font-sans">
                    <p>
                      <strong>1. GUARANTEED RATE FLOOR:</strong> Agaate agrees to purchase certified A-Grade {contractCrop} from the grower at a minimum floor rate of <strong>₹{contract.rate} / kg</strong>.
                    </p>
                    <p>
                      <strong>2. ESTIMATED DELIVERY CAPACITY:</strong> The contract block covering {contractAcres} acres targets a seasonal harvest of <strong>{contract.estYieldKg.toLocaleString()} kg</strong>.
                    </p>
                    <p>
                      <strong>3. LOGISTICS DISPATCH SCHEDULE:</strong> Agaate vehicles will run collection sweeps directly from the Jhajjar Regional Hub every <strong>48 hours</strong> during peak picking.
                    </p>
                    <p className="border-t border-[#E7ECE8]/50 pt-4 text-xs font-mono text-forest font-bold">
                      ESTIMATED FLOOR REVENUE: ₹{contract.minPayout.toLocaleString("en-IN")}
                    </p>
                  </div>
                </div>
              ) : (
                <div className="bg-bone rounded-[2.5rem] border border-[#E7ECE8] p-12 text-center flex flex-col justify-center min-h-[220px]">
                  <FileText className="w-10 h-10 text-forest/30 mx-auto mb-4" />
                  <h4 className="font-serif text-2xl text-forest-deep font-bold mb-1">Contract Draft Standby</h4>
                  <p className="text-xs text-forest/65 max-w-xs mx-auto">
                    Select crop and acreage in the inputs generator to compile purchase baseline floor clauses.
                  </p>
                </div>
              )}
            </div>
